...
glm::vec3 SkeletalAnimation::BoneAnimation::getTranslationAtTick(float tick) const
{
	...
	else
	{
		...
		for (std::vector<PositionKeyFrame>::const_iterator i = posFrames.begin() + 1; i != posFrames.end(); ++i)
		{
			frame1 = frame2;
			frame2 = *i;
...
glm::quat SkeletalAnimation::BoneAnimation::getRotationAtTick(float tick) const
{
	...
	else
	{
		...
		for (std::vector<RotationKeyFrame>::const_iterator i = rotFrames.begin() + 1; i != rotFrames.end(); ++i)
		{
			frame1 = frame2;
			frame2 = *i;
			if (i->getStartTime() > tick)
...
glm::vec3 SkeletalAnimation::BoneAnimation::getScaleAtTick(float tick) const
{
	...
	else
	{
		...
		for (std::vector<ScaleKeyFrame>::const_iterator i = scaleFrames.begin() + 1; i != scaleFrames.end(); ++i)
		{
			frame1 = frame2;
			frame2 = *i;
			if (i->getStartTime() > tick)
