...
glm::vec3 getNormalVector(const Eigen::Matrix4f & B, const Eigen::Vector3f points[], float u) {
...
CKeySpline::CKeySpline() :
	numSplines(0),
	usTableDirty(true),
	VertexBuffer(nullptr),
	IndexBuffer(nullptr)
{
	SetSplineType(ESplineType::Catmull);
}

CKeySpline::~CKeySpline()
{}

void CKeySpline::SetSplineType(ESplineType const t)
{
	switch (t)
	{
	case ESplineType::Catmull:
...
void CKeySpline::AddNode(SSplineNode const & node)
{
...
Transform CKeySpline::TransformAt(float s) const
{
	assert(numSplines > 0);
...
glm::vec3 CKeySpline::NormalAt(float const s) const
{
	float kfloat;
	float uu = s;
	float u = std::modf(uu, &kfloat);
...
float CKeySpline::sToU(float const s)
{
...
void CKeySpline::RecalculateTable(int discretization)
{
	if (numSplines == 0)
		return;
...
int CKeySpline::GetNumNodes()
{
	return (int) nodePos.size();
}

SharedPointer<ion::Graphics::IVertexBuffer> CKeySpline::GetVertexBuffer()
{
...
SharedPointer<ion::Graphics::IIndexBuffer> CKeySpline::GetIndexBuffer()
{
	static SingletonPointer<ion::CGraphicsAPI> GraphicsAPI;
